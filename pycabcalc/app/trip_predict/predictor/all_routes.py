...
class TripPredictor(gp.TripPredictor):
    ...
    def __init__(self, model = model_lin, modelParams=pd.Series([]), limit = None, interval_sz=None):
    ...
    def _loadData(self, ts, te, limit, cols = ['pick_date', 'trip_distance', 'trip_time_in_secs', 'total_wo_tip', 'precip_f', 'precip_b']):
        ...
        if ts.year < 2013:
            ...
            df = self.dbObj.query_Random(limit, date_span = (ts,te), cols=cols)
...
def test():
    s = "Union Square, NYC"
    e = "Times Square, NYC"
    t = datetime.datetime(2013, 12, 4, 13, 33)

    tp = TripPredictor(limit = 5000)
    print tp.getEstimates(s,e,t)
